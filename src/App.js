...
const App = () => {
  ...
  // BACKEND WS URL
  const WS_URL = "wss://driveguard-backend-2.onrender.com/ws/frame";

  // Backend health check
  const checkBackendHealth = async () => {
    try {
      const res = await fetch(
        "https://driveguard-backend-2.onrender.com/api/health"
      );
  ...
  const startMonitoring = async () => {
    ...
    wsRef.current.onmessage = (msg) => {
      ...
      if (data.alert && enableSound) playAlertSound();
    };
  ...
  return (
    <div className="App">
      <header className="app-header">
        <h1>🚗 DriveGuard - Drowsiness Detection</h1>
        <p>Real-time driver monitoring system (WebSocket Mode)</p>
        <div className={`backend-status ${backendStatus}`}>
          Backend: {backendStatus === "online" ? "🟢 Online" : "🔴 Offline"}
        </div>
      </header>
          ...
          <div className="slider-group">
            ...
            <input
              ...
              onChange={(e) =>
                setThresholds({
                  ...thresholds,
                  tilt_threshold: parseInt(e.target.value),
                })
              }
            />
          </div>
        ...
        <div className="main-content">
          ...
          <div className={`status-panel ${alertInfo.alert ? "alert" : "normal"}`}>
            ...
            <div className="metrics-grid">
              <div className="metric-card">
                <span>EAR</span>
                <strong>{alertInfo.metrics?.ear || "0.00"}</strong>
              </div>

              <div className="metric-card">
                <span>Tilt</span>
                <strong>{alertInfo.metrics?.tilt || "0"}°</strong>
              </div>
            </div>

            <small>
              Frames: Eye {alertInfo.metrics?.eye_frames || 0} | Tilt{" "}
              {alertInfo.metrics?.tilt_frames || 0}
            </small>
          </div>
        </div>
      </div>
